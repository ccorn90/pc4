...
nearWinGrid([-,-,-,-,-,-,-,-,-,-,-,-,-,-,-,x,x,o,-,-,-,x,o,x,-,-,-,o,x,o,-,-,-,-,x,o,-,-,-,-,-,-]).
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% hasFollowUpThreat/2 -- true when there is a threat in the row above where a tile will be placed
%% this can be used to keep the computer from setting up a user win
%% it can also help the computer achieve a win by not allowing the user to block one of his threats until forced
hasFollowUpThreat(Grid, Move) :-
    validMove(Grid, Move),
    neighbor(Move,0,-1,FollowUpMove),
    isMoveWinIgnoreVert(Grid, FollowUpMove, x).
    
hasFollowUpThreat(Grid, Move) :-
    validMove(Grid, Move),
    neighbor(Move,0,-1,FollowUpMove),
    isMoveWinIgnoreVert(Grid, FollowUpMove, o).

%% hasFollowUpThreat/3 -- specify tile to look for
hasFollowUpThreat(Grid, Move, Tile) :-
    validMove(Grid, Move),
    neighbor(Move,0,-1,FollowUpMove),
    isMoveWinIgnoreVert(Grid, FollowUpMove, Tile).


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% WARNING: this section does not contain working code
%% trying to make a function that provides all valid moves in random order
randomValidMove(Grid, Move) :-
    
    length(List,Size),
    random(0,Size,Index),
    indexFromList(List,Index,Column),
    removeIndexFromList(List,Index,NewList),
    getMoveInColumn(Grid,Column,Move),
    validMove(Grid,Move).

indexFromList([X|_],0,X).

indexFromList([_|Tail],Index,Element) :-
    NewIndex is Index - 1,
    indexFromList(Tail,NewIndex,Element).

removeIndexFromList([_|Tail],0,Tail).

removeIndexFromList(List,Index,NewList) :-
    rIFL_helper([],List,Index,NewList).

rIFL_helper(Head,[_|Tail],0,NewList) :-
    append(Head,Tail,NewList).
    
rIFL_helper(Head,[X|Tail],Index,NewList) :-
    NewIndex is Index - 1,
    append(Head,[X],NewHead),
    rIFL_helper(NewHead,Tail,NewIndex,NewList).

randomLoop(List) :-
    chooseFromList(List,NL,Col),
    randomLoop(NL).

chooseFromList([X],[],X).

chooseFromList(List,NewList,Elt) :-
    length(List,Size),
    random(0,Size,Index),
    indexFromList(List,Index,Elt),
    removeIndexFromList(List,Index,NewList).
...
% move in a random column only if there is no follow up threat TODO: make loop to randomly select all columns
smarterAI(Grid,_,BestComputerMove) :-
    random(0,6,ColNum),
    getMoveInColumn(Grid,ColNum,BestComputerMove),
    (\+ hasFollowUpThreat(Grid,BestComputerMove) ;
     write('re-run'),smarterAI(Grid,_,BestComputerMove)).

% in case the random fails, move somewhere without a followup threat
smarterAI(Grid,_,BestComputerMove) :-
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% isMoveWinIgnoreVert/3 -- given a CellNumber in a Grid, determine if inserting UsersTile will put the board into a win state
%% we want to ignore the vertical wins here because we are using this in forward thinking AI and dont want to cut off vertical moves
isMoveWinIgnoreVert(Grid, CellNumber, UsersTile) :-
